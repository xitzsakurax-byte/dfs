  ...
  return (
    <div className="min-h-screen bg-[#0A0D14] text-[#F5F7FA] pb-20">
      <div className="container max-w-5xl mx-auto px-6 pt-8">
        ...
        {loading || !stats ? (
          <div className="text-center py-12 text-[#A8B3C7]">Loading your real stats...</div>
        ) : (
          <>
            {/* Current real stats */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
              <div className="practice-card p-5">
                <div className="text-xs text-[#8F95A3]">LEVEL</div>
                <div className="text-5xl font-semibold tracking-tighter mt-1">Lv.{stats.level}</div>
                <div className="text-sm mt-1 text-[#F4C430]">{stats.totalXp} XP total</div>
              </div>
              <div className="practice-card p-5">
                <div className="text-xs text-[#8F95A3]">CURRENT STREAK</div>
              ...
              <div className="practice-card p-5">
                <div className="text-xs text-[#8F95A3]">TODAY (Vietnam)</div>
              ...
              <div className="practice-card p-5">
                <div className="text-xs text-[#8F95A3]">SUGGESTED DAILY GOAL</div>
                <div className="text-3xl font-semibold tracking-tight mt-1">{suggested} words</div>
                <div className="text-xs mt-1 text-[#A8B3C7]">Based on your recent entries</div>
              </div>
            </div>
            ...
            <div className="practice-card p-6 mb-8">
              ...
              <div className="mb-6">
                <div className="text-xs text-[#8F95A3] mb-2">Words mastered per day (last 30)</div>
                  ...
                  {history.slice(0, 30).map((day, idx) => {
                    ...
                    return (
                      <div key={idx} className="flex-1 flex flex-col items-center group">
                        <div 
                          className={`w-full rounded-t ${isToday ? 'bg-[#F4C430]' : 'bg-[#8B1E3D]'} transition-all`}
                          style={{ height: `${height}px` }}
                          title={`${day.date}: ${day.words_mastered} words, ${day.xp_earned} XP`}
                        />
                        <div className="text-[9px] text-[#64748b] mt-1 truncate w-full text-center group-hover:text-[#F4C430]">
                          {day.date.slice(5)}
                        </div>
                      </div>
                    );
                ...
                <div className="space-y-2 text-sm max-h-[420px] overflow-auto pr-2">
                  {history.map((day, idx) => (
                    <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-[#171A21] border border-[#2C303A]">
                      <div>
                        <span className="font-mono text-[#F4C430]">{day.date}</span>
            ...
            <div className="practice-card p-6">
              ...
              <div className="text-xl">
                Over the last 30 days you averaged <span className="font-semibold text-[#F4C430]">{avgWords} words per active day</span>.
              </div>
              <div className="mt-2 text-[#A8B3C7]">
        ...
        <div className="text-center mt-8">
          <Link href="/dashboard" className="text-sm text-[#8F95A3] hover:text-[#EDEEF2]">← Back to main dashboard</Link>
        </div>
      </div>
